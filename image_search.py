...
def fetch_stock_photo_sync(
    query: str, out_dir: str = "gen_images"
) -> Optional[Tuple[str, str]]:
    """Синхронный вариант fetch_stock_photo — для topic-генерации, которая идёт
    в worker-потоке (без event loop). (путь, имя_автора) или None."""
    query = (query or "").strip()
    if not query:
        return None
    if not PEXELS_API_KEY:
