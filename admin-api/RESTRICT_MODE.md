...
## 0. Base URL и сетевой контур

Бэкенд за единым публичным edge — **nginx** (он один публикует 80/443 и терминирует
TLS). `admin-api`, `db`, `rag`, внутренний API бота **не опубликованы наружу**.

- **Продакшн:** фронт обращается к API через домен nginx (напр. `https://api.example.com`),
  а **не** к `:8001` — этот порт теперь привязан к loopback и недоступен снаружи.
  В сборке фронта задаётся `VITE_API_BASE_URL`; на бэкенде origin фронта добавляется
  в CORS через `FRONTEND_URL`.
- **Локальная разработка:** `admin-api` слушает на `127.0.0.1:8001`, фронт по умолчанию
  ходит туда же — ничего настраивать не нужно.
