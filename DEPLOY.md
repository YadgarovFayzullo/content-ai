...
FRONTEND_URL=https://app.ВАШ-ДОМЕН         # origin фронта → CORS admin-api + возврат после OAuth X

# X/Twitter кросс-постинг (опционально; без этих переменных подключение X просто недоступно)
TWITTER_CLIENT_ID=...                      # из X-приложения (OAuth2, тип Web/Confidential)
TWITTER_CLIENT_SECRET=...   
TWITTER_REDIRECT_URI=https://api.ВАШ-ДОМЕН/api/admin/twitter/callback  # тот же URL в настройках X-приложения
TWITTER_TOKEN_KEY=...                      # Fernet-ключ: python -c "from cryptography.fernet import Fernet;print(Fernet.generate_key().decode())"
```
